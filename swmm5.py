...
class PySWMM(object):
    ...
    * swmm_close()
    ...
    def setLinkXsectDiameters(self, index, diameters):
        """
        Set Link Cross-Section Diameters

        :param int index: Link index
        :param list diameters: 4 Xsection diameters
        :return: None
        :rtype: None type
        """
        if diameters.__class__ is list and index.__class__ is int:
            if len(diameters) == 4:
                # Convert to C array pointer
                d = (ctypes.c_double * len(diameters))(*diameters)
            else:
                raise ValueError("Length of diameters must be 4")
        else:
            raise TypeError("index must be integer and diameters must be list")

        errcode = self.SWMMlibobj.swmm_setXsectDiameters(index, d)
        self._error_check(errcode)
    ...
    def getLinkLength(self, index):
        ...
        """
        result = ctypes.c_double()
        errcode = self.SWMMlibobj.swmm_getLinkLength(index,
                                                     ctypes.byref(result))
        self._error_check(errcode)
        return result.value

    def getLinkXsectType(self, index):
        """
        Get Link Type.
        ...
        """
        result = ctypes.c_int()
        errcode = self.SWMMlibobj.swmm_getXsectType(index,
                                                    ctypes.byref(result))
        self._error_check(errcode)
        return result.value

    def getLinkXsectDiameters(self, index):
        """
        Get Link Cross-Section Diameters
        ...
        :rtype: None type
        """
        p = [ctypes.c_double() for i in range(4)]
        e = self.SWMMlibobj.swmm_getXsectDiameters(index, ctypes.byref(p[0]),
                                                          ctypes.byref(p[1]),
                                                          ctypes.byref(p[2]),
                                                          ctypes.byref(p[3]))
        self._error_check(e)
        return [_p.value for _p in p]
        ...
        for attr in dir(object_stats):
            if "_" not in attr:
                ...
                if attr == "timeInFlowClass":
                    ...
                    for ind in range(7):
                        out_dict[object_stats._py_alias_ids[attr]][
                            ind] = stats_array[ind] / (sum_array + 1e-8)
                else:
                    out_dict[object_stats._py_alias_ids[attr]] = getattr(
                        object_stats, attr)
